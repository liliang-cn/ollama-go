...
func main() {
		...
		for {
			...
			case response, ok := <-responsesCh:
				...
				if response.Message.Content != "" {
					if !(response.Message.Thinking != "" || response.Message.Thinking == "") && final {
						fmt.Println("\nFinal result:")
						final = false
					}
	...
	processToolCalls:
		...
		if lastResponse != nil && len(lastResponse.Message.ToolCalls) > 0 {
			for _, toolCall := range lastResponse.Message.ToolCalls {
				var result string
				...
				fmt.Printf("\nCalling tool: %s with arguments: %v\n", toolCall.Function.Name, toolCall.Function.Arguments)
				fmt.Printf("Tool result: %s\n\n", result)

				// Add the assistant message and tool result to the messages
				messages = append(messages, lastResponse.Message)
